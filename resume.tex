...
439 S Stuart Ln, Palatine, IL 60067 \hskip 3cm
847.754.7987}
}
...
\headstyle{
\textbf{Carnegie Mellon University} \hfill 2006 - 2010\\
}
...
Good at \kw{bash}, \kw{HTML 5} and \kw{Javascript} (including
\kw{EcmaScript 6}).
Have worked with \kw{\LaTeX}, \kw{Verilog}, \kw{OpenGL},
\kw{\CSharp}, \kw{Objective-C}, \kw{Perl}, \kw{Ruby}, \kw{SML}, \kw{oCaml},
\kw{Haskell}, \kw{Lisp} and \kw{Scheme}.

\item[\headstyle{Tools}]
Most comfortable with \kw{git}/\kw{vim}/\kw{bash};
also have worked with \kw{XCode}, \kw{Eclipse}, \kw{Visual Studio},
\kw{doxygen}, \kw{mercurial}, \kw{Perforce} and \kw{svn}.

\end{description}

\section{\brass{\textsc{Relevant Work}}}
...
\begin{itemize} 
  ...
  \item Created \kw{HTML5}/\kw{Javascript} dashboard to visualize parsing
...
\item Prepared a technology tutorial to educate a Federal court on technologies
    at issue.
%\item Defended HTC in the \kw{Android} patent wars
\end{itemize}
...
% I researched handwritten and meachine-learned grammars for a novel task and
% processed data on a \kw{Condor} grid.
% \end{position}
%  
...
% \begin{position}
%     Worked with a ten-person team to write \kw{Windows}-based robot control software and robot
% firmware for the \kw{Atmel AVR-8}-based robots as well as build the robots
% themselves.
% \end{position}
...
% usually written in straight \kw{C}.
...
\section{\brass{\textsc{Other Interests}}}
I volunteer with a FIRST Robotics team (WildStang), teaching Java programming
and writing control code for the robot.

In my free time,
I work on projects ranging across \kw{machine translation},
\kw{Android} apps, productivity tools, browser games and \kw{3D printing}, among
other pursuits.
